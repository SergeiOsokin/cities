...
export const CityFormChange = () => {
    ...
    // сохраним новые данные в состоянии
    ...
    // запросим данные по городу при загрузке страницы
    // проблема с Warning: Can't perform a React state update on an unmounted component.
    useEffect(() => {
        getCity();
        return () => {
            return false;
    ...
    // отправим измененные данные на сервер  
    ...
    return (
        <>
            <form className="formCity" onSubmit={submitHandler}>
                <p className="titleNewCity">Изменить город</p>
                <fieldset>
                        ...
                        {colorAvailable.map((item, index) => {
                            return (
                                <div className="container-color" key={item}>
                                    ...
                                    />
                                    <label
                                        className={`colorLabel ${item === city.color && 'color_checked'}`}
